...
class Locale {
  ...
  static create (locale, numberingSystem, outputCalendar, defaultToEN = false) {
    ...
    const localeR = specifiedLocale || (defaultToEN
? 'en-US'
: systemLocale());
  ...
  constructor (locale, numbering, outputCalendar, specifiedLocale) {
    const [
parsedLocale,
parsedNumberingSystem,
parsedOutputCalendar
...
'standalone': {}};
  ...
  listingMode () {
    ...
    return isActuallyEn && hasNoWeirdness
? 'en'
: 'intl';
  }
...
function untruncateYear (year) {
  if (year > 99) {
    return year;
  } return year > Settings.twoDigitCutoffYear
? 1900 + year
: 2000 + year;
}
...
function parseZoneInfo (ts, offsetFormat, locale, timeZone = null) {
  ...
  const modified = {'timeZoneName': offsetFormat,
...intlOpts};
    ...
    for (let i = 0; i < fmt.length; i++) {
      ...
      if (c === '\'') {
        if (currentFull.length > 0) {
          splits.push({'literal': bracketed || (/^\s+$/).test(currentFull),
'val': currentFull});
        }
        current = null;
        currentFull = '';
        ...
        if (currentFull.length > 0) {
          splits.push({'literal': (/^\s+$/).test(currentFull),
'val': currentFull});
        }
        currentFull = c;
        current = c;
...
: '';
      ...
      meridiem = () => (knownEnglish
          ...
          : string({hour: 'numeric',
hourCycle: 'h12' }, 'dayperiod')),
      month = (length, standalone) => (knownEnglish
          ? monthForDateTime(dt, length)
          : string(standalone
? {month: length}
: {month: length,
day: 'numeric' }, 'month')),
      weekday = (length, standalone) => (knownEnglish
...
'allowZ': this.opts.allowZ});
          case 'ZZZ':
            // Like +0600
            return formatOffset({'format': 'techie',
'allowZ': this.opts.allowZ});
          case 'ZZZZ':
            // Like EST
            return dt.zone.offsetName(dt.ts, {'format': 'short',
'locale': this.loc.locale});
          case 'ZZZZZ':
            // Like Eastern Standard Time
            return dt.zone.offsetName(dt.ts, {'format': 'long',
'locale': this.loc.locale});
          // Zone
          case 'z':
...
: this.num(dt.day, 2);
          ...
          case 'L':
            // Like 1
            return useDateTimeFormatter
              ? string({'month': 'numeric',
'day': 'numeric'}, 'month')
              : this.num(dt.month);
          case 'LL':
            // Like 01, doesn't seem to work
            return useDateTimeFormatter
              ? string({'month': '2-digit',
'day': 'numeric'}, 'month')
              : this.num(dt.month, 2);
          case 'LLL':
            // Like Jan
...
millisecondsStr
  ...
  const maybeNegate = (num, force = false) => (num !== undefined && (force || num && hasNegativePrefix)
? -num
: num);
...
: typeof obj
  ...
  static invalid (reason, explanation = null) {
    ...
    const invalid = reason instanceof Invalid
? reason
...
'unit': unit.slice(0, -1)}).
    ...
    return this.loc.
      listFormatter({'type': 'conjunction',
...
...normalizeObject(values, Duration.normalizeUnit)};
  ...
  reconfigure ({locale, numberingSystem, conversionAccuracy, matrix} = {}) {
    const loc = this.loc.clone({locale,
numberingSystem});
    const opts = {loc,
matrix,
conversionAccuracy};
...
negate();
 ...
 return this;
    ...
    const vals = removeZeroes(this.normalize().shiftToAll().
toObject());
    return clone$1(this, {'values': vals}, true);
  }
...
class Info {
  ...
  static hasDST (zone = Settings.defaultZone) {
    const proto = DateTime.now().setZone(zone).
set({'month': 12});
...
function dayDiff (earlier, later) {
  const utcDayStart = (dt) => dt.toUTC(0, {'keepLocalTime': true}).startOf('day').
valueOf(),
...
highWater,
lowestOrder
] = highOrderDiffs(earlier, later, units);
...
function unitForToken (token, loc) {
  const one = digitRegex(loc),
    ...
    literal = (t) => ({'regex': RegExp(escapeToken(t.val)),
'deser': ([s]) => s,
'literal': true}),
    unitate = (t) => {
      if (token.literal) {
...
: 'hour24';
       ...
       */
      actualType = resolvedOpts.hour12
? 'hour12'
: 'hour24';
    }
  }
...
function uncomputeOrdinal (year, ordinal) {
  const table = isLeapYear(year)
? leapLadder
: nonLeapLadder,
    month0 = table.findIndex((i) => i < ordinal),
    day = ordinal - table[month0];
  return {'month': month0 + 1,
day};
}
...
ts,
o
] = fixOffset(localTS, oPre, inst.zone);
...
function toISODate (o, extended) {
 ...
 c += '+';
  ...
  c += padStart(o.c.year, longFormat
? 6
: 4);
...
function diffRelative (start, end, opts) {
  const round = isUndefined(opts.round)
? true
: opts.round,
    format = (c, unit) => {
...
? -0
: 0, opts.units[opts.units.length - 1]);
}
...
tsFinal,
offsetFinal
] = objToTS(gregorian, offsetProvis, zoneToUse),
      inst = new DateTime({
        'ts': tsFinal,
        'zone': zoneToUse,
        'o': offsetFinal,
...
specificOffset,
invalid
] = parseFromTokens(localeToUse, text, fmt);
    if (invalid) {
      return DateTime.invalid(invalid);
    }
...
ts,
o
] = objToTS(mixed, this.o, this.zone);
...
: NaN);
  ...
  diff (otherDateTime, unit = 'milliseconds', opts = {}) {
    ...
    const durOpts = {'locale': this.locale,
